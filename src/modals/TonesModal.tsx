...
const TonesModal = ({setShowTones}:MainProps) => {
          ...
          <div className="flex flex-col gap-2">
            {tones.map((tone) => (
              <button
                ...
                className="w-full flex items-center justify-between rounded-xl bg-gray-800 shadow-lg  px-4 py-2 text-sm text-gray-200 transition hover:bg-gray-700/70 hover:text-white focus:outline-none focus:ring-2 focus:ring-indigo-500"
              >
                <span>{tone.name}</span>
                {tone.current && (
                  <Check
            ...
            <p onClick={()=>setShowTones(false)} className="text-center active:underline cursor-pointer text-sm mt-1 text-red-400 font-medium">cancel</p>
          </div>
        </div>
      </div>
  );
